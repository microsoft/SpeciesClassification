...
# for training and inference. In contrast to data_loader.py, OpenCV is used for loading images.
#
################
...
class ImageLoader():
    ...
    def load_image(self, path):
        ...
        cv2_img = cv2.cvtColor(cv2.imread(path), cv2.COLOR_BGR2RGB)
        pil_img = Image.fromarray(cv2_img)
        return pil_img 

    def process_image(self, img, is_train, multi_crop = False, bboxes = [], showImage = False):
        ...
        # In validation
            ...
            resized_img_cv = cv2.resize(np.array(img),
                                        (int(img.size[0]*scale_ratio),
