...
def generate_detections(detection_graph, dir, images):
    """
    ...
    with detection_graph.as_default():
        with tf.Session(graph=detection_graph) as sess:
            for iImage,image in enumerate(images):                
                ...
                imageNP = mpimg.imread(dir + image)

                if (len(imageNP.shape) == 2):
                    imageNP = np.expand_dims(imageNP, 3)
                    imageNP = np.tile(imageNP, [1, 1, 3])
